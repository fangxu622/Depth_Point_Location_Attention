...
for e in range(1,config.epochs+1):
    ...
    for i, (depth_base , _ ,  base_t,base_q) in enumerate(train_loader):
        ...
        t_pred, q_pred = model(depth_input)
    ...
    if (e > -1 and e % config.interval == 0):
        ...
        with torch.no_grad():
            ...
            for _ , (depth_base , _ , base_t,base_q) in enumerate(tqdm(test_loader) ):
                depth_input = depth_base.to(device)
                # pcd_input = {e: pcd_base[e].to(device) for e in pcd_base}
                t_gt, q_gt = base_t.to(device) , base_q.to(device)

                t_infer, q_infer = model(depth_input)
